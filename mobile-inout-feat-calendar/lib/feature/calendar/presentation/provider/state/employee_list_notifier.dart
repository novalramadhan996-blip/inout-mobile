...
class EmployeeListEventsNotifier
    extends StateNotifier<BaseState<List<OrganizationEmployee>>> {
  final CalendarRepository calendarRepository;

  EmployeeListEventsNotifier(this.calendarRepository)
    : super(const BaseState<List<OrganizationEmployee>>());

  bool get isFetching =>
      state.state != ConcreteState.loading &&
      state.state != ConcreteState.fetchingMore;

  Future<void> getEmployeeList(ListDataRequest params) async {
    ...
    final response = await calendarRepository.getEmployeeList(params);
